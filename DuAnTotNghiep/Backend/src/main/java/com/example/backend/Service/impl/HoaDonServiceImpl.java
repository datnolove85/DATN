...
public class HoaDonServiceImpl implements HoaDonService {
    ...
    public  HoaDonDetailResponse getDetail(Integer id) {
        ...
        // 1. Khởi tạo response lên đầu để có thể dùng các hàm setter
        ...
        if (hd.getIdKhachHang() != null) {
            response.setIdKhachHang(hd.getIdKhachHang().getId());
       ...
       if(hd.getIdNhanVien() != null) {
        ...
        response.setSoXuSuDung(hd.getSoXuSuDung());
        response.setTienGiamDoXuSuDung(hd.getTienGiamDoXu());
        response.setTongTienHang(hd.getTongTienHang());
        response.setTongGiamGia(hd.getTongGiamGia());
        response.setPhiVanChuyen(hd.getPhiVanChuyen());
        response.setTongThanhToan(hd.getTongThanhToan());
        response.setNgayTao(hd.getNgayTao());
        response.setNgayCapNhat(hd.getNgayCapNhat());

        response.setSanPhams(sanPhams);

        // 2. Lấy phương thức thanh toán
        String phuongThuc = "";
        Optional<ThanhToan> thanhToan = ttRepo.findFirstByIdHoaDon_Id(id);
        if (thanhToan.isPresent()) {
            phuongThuc = thanhToan.get().getIdPhuongThucThanhToan().getTenPhuongThuc();
        }
        response.setPhuongThucThanhToan(phuongThuc);

        // 3. Lấy và set lịch sử hóa đơn
        List<LichSuHoaDon> listEntity = lichSuHoaDonRepository.findByHoaDonIdCustom(id);
        System.out.println("DEBUG: Số lượng bản ghi tìm thấy: " + (listEntity != null ? listEntity.size() : "NULL"));

        if (listEntity == null) {
            listEntity = new ArrayList<>();
        }

        List<LichSuHoaDonResponse> lichSuHoaDons = listEntity.stream()
                .map(ls -> LichSuHoaDonResponse.builder()
                        .id(ls.getId())
                        .trangThaiCu(ls.getTrangThaiCu())
                        .trangThaiMoi(ls.getTrangThaiMoi())
                        .thoiGian(ls.getThoiGian())
                        .tenNhanVien(ls.getNhanVien() != null ? ls.getNhanVien().getTenNhanVien() : null)
                        .ghiChu(ls.getGhiChu())
                        .nguonThaoTac(ls.getNguonThaoTac())
                        .build()
                )
                .toList();

        response.setLichSuHoaDons(lichSuHoaDons);

        // 4. Lấy voucher (nếu có)
        Optional<HoaDonVoucher> hdVoucherOpt = hoaDonVoucherRepo.findByIdHoaDon_Id(id);

        if (hdVoucherOpt.isPresent()) {

            HoaDonVoucher hdVoucher = hdVoucherOpt.get();

            HoaDonVoucherResponse voucher = new HoaDonVoucherResponse();
            voucher.setSoTienGiam(hdVoucher.getSoTienGiam());
            ...
            if (hdVoucher.getIdVoucher() != null) {
                ...
                voucher.setTenVoucher(v.getTenVoucher());
                voucher.setMaCode(v.getMaVoucher());
                voucher.setLoaiGiamGia(v.getLoaiGiamGia());
                voucher.setGiaTriGiam(v.getGiaTriGiam());
                voucher.setGiaTriGiamToiDa(v.getGiaTriGiamToiDa());
                voucher.setDieuKienToiThieu(v.getGiaTriDonHangToiThieu());
                voucher.setLoaiVoucher("HE_THONG");
            }
            ...
            // Voucher minigame
                ...
                voucher.setId(kv.getId());
                voucher.setTenVoucher(kv.getTenVoucher());
                voucher.setMaCode(kv.getMaCode());
                voucher.setLoaiGiamGia(kv.getLoaiGiamGia());
                voucher.setGiaTriGiam(kv.getGiaTriGiam());
                voucher.setGiaTriGiamToiDa(kv.getGiaTriGiamToiDa());
                voucher.setDieuKienToiThieu(kv.getDieuKienToiThieu());
                voucher.setLoaiVoucher("MINIGAME");
            }
    ...
    public Object thanhToanHoaDon(ThanhToanHoaDonRequest req, Integer idNhanVien) {
        ...
        thayDoiTrangThai(hd.getId(), "hoan_thanh", "Thanh toán hóa đơn tại quầy", "STAFF", idNhanVien);
...
// ==================================================================================
    ...
    public Object thanhToanHoaDonOnline(
        ...
        if ("COD".equals(pt.getMaPhuongThuc())) {
            ...
            thayDoiTrangThai(hd.getId(), "cho_xac_nhan", "Đặt hàng COD online", "CUSTOMER", null);
            ...
            hd.setTrangThaiThanhToan("da_thanh_toan");
            thayDoiTrangThai(hd.getId(), "da_xac_nhan", "Thanh toán online thành công", "CUSTOMER", null);
        }
    ...
    public List<VoucherKhachHangResponse> ganKhachHang(Integer idHoaDon, Integer idKhachHang) {
        ...
        for (VoucherCuaKhachHang vckh : list) {
            ...
            KhoVoucher kho = khoVoucherRepository.findById(vckh.getIdVoucherMinigame())
                    .orElse(null);
    ...
    public void recalculateHoaDon(Integer idHoaDon) {
        ...
        // 1. Tính tiền giảm từ Voucher (Hệ thống hoặc Kho voucher)
        BigDecimal tongGiamGiaVoucher = BigDecimal.ZERO;
        HoaDonVoucher hdVoucher = hoaDonVoucherRepo.findByIdHoaDon_Id(idHoaDon)
                .orElse(null);

        if (hdVoucher != null) {
            if (hdVoucher.getIdVoucher() != null) {
                tongGiamGiaVoucher = tinhTienGiam(
                        hdVoucher.getIdVoucher(),
                        tongTienHang
                );
            } else if (hdVoucher.getIdKhoVoucher() != null) {
            ...
            hdVoucher.setSoTienGiam(tongGiamGiaVoucher);
            hoaDonVoucherRepo.save(hdVoucher);
        }

        // 2. Lấy thêm tiền giảm từ Xu (nếu khách có chọn dùng xu)
        BigDecimal tienGiamDoXu = hd.getTienGiamDoXu() != null
                ? hd.getTienGiamDoXu()
                : BigDecimal.ZERO;

        // 3. Tổng giảm giá = Giảm giá Voucher + Giảm giá từ Xu
        BigDecimal tongGiamGia = tongGiamGiaVoucher.add(tienGiamDoXu);

        // Ràng buộc: Tổng giảm giá không được vượt quá tổng tiền hàng
        if (tongGiamGia.compareTo(tongTienHang) > 0) {
            tongGiamGia = tongTienHang;
        }
        ...
        // 4. Tính lại tổng thanh toán cuối cùng
    ...
    public void apVoucher(Integer idHoaDon, Integer idVoucher, Integer idVoucherKhachHang) {
        ...
        // ==============================
            ...
            khoVoucherToUse = khoVoucherRepository.findById(voucherKhachToUse.getIdVoucherMinigame())
...
// 4. Áp voucher (hệ thống hoặc voucher khách hàng)
        ...
        if (req.getSoXuSuDung() != null && req.getSoXuSuDung() > 0) {
            apDungXu(hd.getId(), req.getSoXuSuDung());
        }
    ...
    public void updateTrangThai(
        ...
        if (newStatus == TrangThaiHoaDon.DA_HUY
                || newStatus == TrangThaiHoaDon.GIAO_THAT_BAI) {
        ...
        if (newStatus == TrangThaiHoaDon.GIAO_THANH_CONG) {
            ...
            tichXuVaNangHangService.xuLyHoanTatDonHang(hoaDon.getId());
        }
        ...
        switch (newStatus) {
            ...
            case DA_XAC_NHAN:
                ...
                break;
            case CHO_VAN_CHUYEN:
                ghiChu = "Chờ vận chuyển";
                break;
            case DANG_GIAO:
                ghiChu = "Đang giao hàng";
                break;
            case GIAO_THANH_CONG:
                ghiChu = "Giao hàng thành công";
                break;
            case GIAO_THAT_BAI:
                ghiChu = "Giao hàng thất bại";
                break;
            case HOAN_THANH:
    ...
    @Transactional
    public void thayDoiTrangThai(Integer idHoaDon, String trangThaiMoi, String ghiChu, String nguonThaoTac, Integer idNhanVien) {
        // 1. Lấy hóa đơn cũ
        ...
        // Nếu là nhân viên thì set id nhân viên
        if (idNhanVien != null) {
    ...
    @Transactional
    public void apDungXu(Integer idHoaDon, Integer soXuSuDung) {
        HoaDon hd = hoaDonRepo.findById(idHoaDon)
                .orElseThrow(() -> new RuntimeException("Không tìm thấy hóa đơn"));

        if (hd.getIdKhachHang() == null) {
            throw new RuntimeException("Hóa đơn chưa được gán khách hàng, không thể dùng xu!");
        }

        KhachHang kh = hd.getIdKhachHang();
        int soDuHienTai = kh.getSoDuXu() != null ? kh.getSoDuXu() : 0;
        ...
        BigDecimal tyLeQuyDoi = cauHinhHeThongRepository.findByMaCauHinh(maQuyDoiXu)
                .map(CauHinhHeThong::getGiaTriSo)
                .orElse(BigDecimal.valueOf(1000));

        BigDecimal tyLeGiamToiDa = cauHinhHeThongRepository.findByMaCauHinh(maGiamToiDaXu)
                .map(CauHinhHeThong::getGiaTriSo)
