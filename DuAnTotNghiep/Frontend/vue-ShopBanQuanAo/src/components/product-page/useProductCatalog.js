...
export function useProductCatalog(options = {}) {
  ...
  const applyRouteFilters = () => {
    ...
    const queryKeyword = String(route.query.keyword || '').trim()

    if (queryKeyword) {
      filters.keyword = queryKeyword
    }

    if (!queryCategory || !catalogOptions.categories.length) return

    const normalized = queryCategory.toLowerCase()
    const matched = catalogOptions.categories.find((item) => {
      const id = String(item.id ?? '').toLowerCase()
      const code = String(item.maDanhMuc ?? '').toLowerCase()
      const name = String(item.name ?? '').toLowerCase()
      return id === normalized || code === normalized || name === normalized
    })

    if (matched) {
      filters.categories = [matched.name]
      currentPage.value = 1
    }
  }
