  ...
  >
    ...
    >
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-base font-bold text-slate-800">Quản lý thương hiệu</h1>
          <p class="text-slate-400 text-[11px]">
            Quản lý danh sách thương hiệu, thông tin mô tả và trạng thái hoạt động.
          </p>
        </div>
      </div>
    ...
    <div class="space-y-3 overflow-hidden">
      ...
      >
        ...
        <div class="col-span-2 px-4 border-r border-slate-100">Mã thương hiệu</div>
        <div class="col-span-3 px-4 border-r border-slate-100">Tên thương hiệu</div>
      ...
      <div class="space-y-2">
            ...
            >
              <span
                :class="[
                  ...
                  brand.trangThai ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500',
                ]"
              ></span>
        ...
        >
          <p>Kho lưu trữ thương hiệu trống rỗng.</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="deleteId" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      >
        ...
        <p class="text-[11px] text-slate-400 mb-6">Bạn có chắc chắn muốn xóa thương hiệu này?</p>
...
const handleSubmit = async (): Promise<void> => {
  const isDuplicateMa = brands.value.some(
    (item) =>
      item.maThuongHieu.trim().toLowerCase() === form.value.maThuongHieu.trim().toLowerCase() &&
      (!isEditMode.value || item.id !== form.value.id),
  )

  const isDuplicateTen = brands.value.some(
    (item) =>
      item.tenThuongHieu.trim().toLowerCase() === form.value.tenThuongHieu.trim().toLowerCase() &&
      (!isEditMode.value || item.id !== form.value.id),
  )

  if (isDuplicateMa) {
    showToast('Mã thương hiệu này đã tồn tại!', 'error')
    return
  }

  if (isDuplicateTen) {
    showToast('Tên thương hiệu này đã tồn tại!', 'error')
    return
  }

  try {
    if (isEditMode.value) {
      await updateThuongHieu(form.value.id!, form.value)
      showToast('Cập nhật thành công!')
    } else {
      await createThuongHieu(form.value)
...
const promptDelete = (id: number | null): void => {
  deleteId.value = id
}

const confirmDelete = async (): Promise<void> => {
  try {
    await deleteThuongHieu(deleteId.value!)
    await loadData()
