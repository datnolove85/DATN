  ...
  >
    ...
    <div class="space-y-3 overflow-hidden">
      ...
      <div class="space-y-2">
        ...
        >
          <p class="text-slate-700 font-bold text-sm">Kho lưu trữ chất liệu trống rỗng</p>
        </div>
      </div>
    </div>
      ...
      >
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <input
            ...
            placeholder="Mã chất liệu (VD: CT)"
...
const handleSubmit = async () => {
  try {
    if (isEditMode.value) await updateChatLieu(form.value.id, form.value)
    else await createChatLieu(form.value)
    await loadData()
    closeModal()
  } catch (err) {
    console.error(err)
